...
def format_license(text):
    ...
    # Insert dash and dot for the required format
    if len(license_plate_) == 8:
        license_plate_ = '{}{}{}-{}.{}'.format(license_plate_[0], license_plate_[1], license_plate_[2],
                                               license_plate_[3:6], license_plate_[6:])
...
def read_license_plate(license_plate_crop):
    """
    Đọc văn bản biển số xe từ ảnh đã cắt.

    Args:
        license_plate_crop (PIL.Image.Image): Ảnh đã cắt chứa biển số xe.

    Returns:
        tuple: Chuỗi văn bản biển số xe đã định dạng và điểm tin cậy của nó.
    """
    ...
    # Duyệt qua từng kết quả nhận diện
    for detection in detections:
        bbox, text, score = detection

        # Chuyển văn bản sang chữ hoa và loại bỏ các ký tự không cần thiết
        text = text.upper().replace(' ', '').replace('.', '').replace('-', '').replace('_', '')

        # Hợp nhất kết quả nhận diện
        merged_text += text
        highest_score = max(highest_score, score)

    if license_complies_format(merged_text):
        return format_license(merged_text), highest_score
